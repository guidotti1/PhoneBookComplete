...
function buildTable(list) {
	...
	for (var i = 1; i < aLen; i+=5) {
	    result += "<tr id="' + i + '"><td class='first'>"+a[i]+"</td><td class='last'>"+a[i+1]+"</td><td class='phone'>"+a[i+2]+"</td><td class='type'>"+a[i+3]+"</td>";
...
function editEntry(){
    ...
    $('#searchresults').empty();
    $.ajax({
	url: '/cgi-bin/guidotti1_phoneAppComplete.cgi?editfname='+$('#editfirst').val()+'&editlname='+$('#editlast').val()+'&editphone='+$('#editphone').val()+'&edittype='+$('#edittype').text()+'&operation=edit',
	dataType: 'text',
	success: editDone(),
